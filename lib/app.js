...
function findAll(session, orgId, callback) {
	...
	Request.createRequest(session, '/api/v1/app' + (orgId ? ('?org_id=' + orgId) : ''), callback);
};
...
function find(session, appId, callback) {
	Request.createRequest(session, '/api/v1/app/' + appId, callback);
};
...
function update(session, app, callback) {
	...
	Request.createRequest(session, '/api/v1/app/' + guid, 'put', callback, app);
};
...
function crittercismID(session, guid, callback) {
	Request.createRequest(session, '/api/v1/app/' + guid + '/crittercism_id', callback);
};

function updateTiApp(session, orgId, tiappxml, callback) {
	var req = Request.createRequest(session, '/api/v1/app/saveFromTiApp?org_id=' + orgId, 'post', callback);
	if (req) {
		var form = req.form();
...
function findPackage(session, guid, token, callback) {
	...
	Request.createRequest(session || token, '/api/v1/app/' + guid + '/package', callback);
};
...
function findTeamMembers(session, guid, callback) {
	Request.createRequest(session, '/api/v1/app/' + guid + '/team', callback);
};
...
function deleteApp(session, appId, callback) {
	Request.createRequest(session, '/api/v1/app/' + appId, 'del', callback);
};
