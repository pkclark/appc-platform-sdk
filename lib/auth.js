...
Auth.createSessionFromID = function createSessionFromID(id, callback) {
	...
	async.series([
		...
		AppC.createRequest(session, '/api/v1/auth/findSession', function (err, body, res) {
			...
			entitlements.partners = ['acs', 'analytics'];

			if (body.org.limit_performance_users && body.org.limit_performance_users > 0) {
				entitlements.partners.push('crittercism');
			}

			if (body.org.limit_test_users && body.org.limit_test_users > 0) {
				entitlements.partners.push('soasta');
			}
