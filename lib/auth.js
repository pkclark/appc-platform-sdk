...
Auth.logout = function (session, callback) {
	AppC.createRequest(session, '/api/v1/auth/logout', 'get', function (e) {
		session._invalidate();
		callback && callback(e);
	});
};
 ...
 * login
 */
Auth.login = function (username, password, deviceid, callback) {
	var self = this;
	if (typeof(deviceid) === 'function') {
		if (cachedMac) {
			return this.login(username, password, cachedMac, deviceid);
		}
		return Mac.getMac(function (err, macAddress) {
			cachedMac = macAddress || '00-00-00-00-00-00';
			return self.login(username, password, cachedMac, deviceid);
		});
	}
		...
		checkError = function (err, result) {
			if (err) {
				...
				if (err.code) {
					switch (err.code) {
						case 'ECONNREFUSED':
							return callback(makeError('Connection refused to ' + AppC.baseurl, self.ERROR_CONNECTION_REFUSED));
						case 'ECONNRESET':
							return callback(makeError('Connection reset to ' + AppC.baseurl, self.ERROR_CONNECTION_RESET));
						case 'CERT_HAS_EXPIRED':
							return callback(makeError('The servers SSL certificate at ' + AppC.baseurl + ' has expired. Refusing to connect.', self.ERROR_CONNECTION_INVALID_SSL));
						case 400:
							return callback(makeError(err, self.ERROR_CONNECTION_SERVER_ERROR));
					}
				}
				return callback(makeError(err, self.ERROR_CONNECTION_SERVER_ERROR));
			}
...
Auth.createSessionFromRequest = function (req, callback) {
	if (!req.cookies) {
		return callback(makeError('not logged in', this.ERROR_NOT_AUTHORIZED));
	}
	var id = req.cookies['dashboard.sid'] || req.cookies['connect.sid'];
	if (!id) {
		return callback(makeError('not logged in', this.ERROR_NOT_AUTHORIZED));
	}
	return this.createSessionFromID(id, callback);
};

/**
 * from an existing authenticated session, create a new Session object
 */
Auth.createSessionFromID = function (id, callback) {
	...
	async.series([
		...
		AppC.User.find(session, function (err, user) {
			if (err && err.code === 403) {
				return callback(makeError('invalid session', self.ERROR_NOT_AUTHORIZED));
			}
			if (err) {
				return callback(err);
			}
 ...
 */
Auth.requestLoginCode = function (session, sms, callback) {
	var self = this;
	AppC.User.find(session, function (err, user) {
		if (err) {
			return callback(err);
		...
		if (user.disable_2fa) {
			return callback(makeError('Two-factor authentication is disabled', self.ERROR_TWOFACTOR_DISABLED));
		}
		if (sms && !user.phone) {
			return callback(makeError('No SMS number configured. Please configure your SMS number in your profile to use SMS verification.', self.ERROR_NO_PHONE_CONFIGURED));
		}
 ...
 */
Auth.validateSession = function (object, callback) {
 ...
 */
Auth.verifyLoginCode = function (session, code, callback) {
	var self = this,
		r = AppC.createRequest(session, '/api/v1/auth/deviceauth', 'post', function (err, result) {
			if (err) {
				return callback(err);
			}
			if (result.expired) {
				return callback(makeError('Your authorization code has expired.', self.ERROR_AUTH_CODE_EXPIRED));
			}
			if (!result.valid) {
				return callback(makeError('Your authorization code was invalid.', self.ERROR_AUTH_CODE_INVALID));
			}
 ...
 */
Auth.cacheSession = function (session) {
	if (session) {
