...
function login(params, callback) {
	...
	function checkError(err, result, resp) {
		if (err) {
			...
			if (err.code) {
				switch (err.code) {
					case 'ECONNREFUSED':
						return callback(_makeError('Connection refused to ' + Env.baseurl, ERRORS.ERROR_CONNECTION_REFUSED), null, resp);
					case 'ECONNRESET':
						return callback(_makeError('Connection reset to ' + Env.baseurl, ERRORS.ERROR_CONNECTION_RESET), null, resp);
					case 'CERT_HAS_EXPIRED':
						return callback(_makeError('The servers SSL certificate at ' + Env.baseurl + ' has expired. Refusing to connect.', ERRORS.ERROR_CONNECTION_INVALID_SSL), null, resp);
					case 400:
						return callback(_makeError(err, ERRORS.ERROR_CONNECTION_SERVER_ERROR), null, resp);
					default:
						break;
				}
			}
	...
	let r = Request.createRequest(session, '/api/v1/auth/login', 'post', checkError, mapper);
	let form = r.form();
...
function requestLoginCode(session, sms, callback) {
	User.find(session, function (err, user, res) {
		if (err) {
...
function verifyLoginCode(session, code, callback) {
	var r = Request.createRequest(session, '/api/v1/auth/deviceauth', 'post', function (err, result, resp) {
		if (err) {
...
function _resolveUserOrg(session, next) {
	...
	Org.find(session, function (err, orgs, resp) {
		if (err) {
