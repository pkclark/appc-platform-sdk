	...
	function checkError(err, result, resp) {
		if (err) {
			...
			if (err.code) {
				switch (err.code) {
					case 'ECONNREFUSED':
						return callback(_makeError('Connection refused to ' + Env.baseurl, ERROR_CONNECTION_REFUSED), null, resp);
					case 'ECONNRESET':
						return callback(_makeError('Connection reset to ' + Env.baseurl, ERROR_CONNECTION_RESET), null, resp);
					case 'CERT_HAS_EXPIRED':
						return callback(_makeError('The servers SSL certificate at ' + Env.baseurl + ' has expired. Refusing to connect.', ERROR_CONNECTION_INVALID_SSL), null, resp);
					case 400:
...
function switchLoggedInOrg(session, orgId, callback) {
	Org.getById(session, orgId, function (err, org) {
		...
		let req = Request.createRequest(session, '/api/v1/auth/switchLoggedInOrg', 'post', callback, function mapper(obj, next, resp) {
			...
			cachedUser = null;
			cachedUserKey = null;
			createSessionFromID(sid, function (err, newsession) {
				if (err) {
					return next(err);
				}
