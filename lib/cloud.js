...
function login(session, callback) {
	try {
		var url = module.exports.getEnvironment(session) + '/v1/admins/login360.json?ct=enterprise&connect.sid=' + session.id;
 ...
 */
module.exports = {
	NODE_ACS: 'nodeACSEndpoint',
	ACS_BASE: 'acsBaseUrl',
	AUTH_BASE: 'acsAuthBaseUrl',
	/**
	 ...
	 */
	createNamedApp: function (session, name, callback) {
		var self = this;
		try {
			var tasks = [],
				apps;
			...
			tasks.push(function (next) {
				var url = self.getEnvironment(session) + '/v1/apps/create.json?_session_id=' + session.acs_session + '&ct=enterprise';
	 ...
	 * @param {Function} callback
	 */
	createApp: function (session, appName, orgId, appGuid, callback) {
	 ...
	 * @param {Function} callback
	 */
	createUser: function (session, guid, keyvalues, callback) {
	 ...
	 */
	getEnvironment: function (session, type, name) {
		if (!session.user) {
	 ...
	 * @param {Function} callback
	 */
	retrieveUsers: function (session, guid, callback) {
		var req = AppC.createRequest(session, '/api/v1/acs/' + guid + '/data.next/user', createACSResponseHandler('users', callback));
	}
};