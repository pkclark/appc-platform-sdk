...
function login(session, callback) {
	try {
		...
		Request.createRequestCustomResponseHandler(session, url, createCloudResponseHandler(function (err, body) {
			if (!err && body) {
...
function createNamedApp(session, name, callback) {
	try {
		...
		tasks.push(function (next) {
			...
			var r = Request.createRequestCustomResponseHandler(session, url, 'post', createCloudResponseHandler(function (err, body) {
				if (err) {
					return next(err);
				}
		...
		tasks.push(function (next) {
			async.each(apps, function (app, cb) {
				var req = Request.createRequestCustomResponseHandler(session, '/api/v1/api', 'post', createACSResponseHandler('apps', function (err, result) {
					if (err) {
						return next(err);
					}
					if (result.length === 0) {
...
function createApp(session, appName, orgId, appGuid, callback) {
	var req = Request.createRequest(session, '/api/v1/acs', 'post', createACSResponseHandler('apps', callback));
	if (req) {
		var form = req.form();
		if (appGuid) {
...
function createUser(session, guid, keyvalues, callback) {
	let req = Request.createRequest(session, '/api/v1/acs/' + guid + '/data.next/user', 'post', createACSResponseHandler('users', callback));
	if (req) {
		let form = req.form();
...
function getEnvironment(session, type, name) {
	...
	type = type || ACS_BASE;
	name = name || Env.isProduction ? 'production' : 'development';
...
function retrieveUsers(session, guid, callback) {
	Request.createRequest(session, '/api/v1/acs/' + guid + '/data.next/user', createACSResponseHandler('users', callback));
};
