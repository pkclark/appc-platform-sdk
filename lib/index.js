 ...
 */
function _createRequestOpts(session, url, authToken) {
	...
	// Only set keep-alives if no proxy and strictSSL is expected.
	if (!opts.proxy && opts.strictSSL !== false && opts.agent !== false) {
		opts.forever = true;
	}
...
function _createRequest(session, path, method, responseHandler, json) {
	try {
		...
		let opts = _createRequestOpts(session, path);
		if (json) {
			opts.json = json;
		}
