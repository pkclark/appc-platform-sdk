...
var AppC = require('./index'),
	...
	analyticsDir = path.join(os.homedir && os.homedir() || process.env.HOME || process.env.HOMEPATH || process.env.USERPROFILE, '.appc-analytics'),
	timer,
	sending,
	sequence = 1;
...
Analytics.startSendingEvents = function () {
	debug('stopSendingEvents', Analytics.flushInterval);
 ...
 * @private
 */
function Session (guid, mid, sid, deploytype, os) {
	this.guid = guid;
	this.mid = mid;
	this.sid = sid;
...
Session.prototype.send = function (eventdata, event) {
	Analytics.sendEvent(this.guid, this.mid, eventdata, event, this.deploytype, this.sid, this.os);
};
...
Session.prototype.end = function (data) {
	Analytics.sendEvent(this.guid, this.mid, data, 'ti.end', this.deploytype, this.sid, this.os);
};
 ...
 */
Analytics.createSession = function sendEvent (guid, mid, eventdata, deploytype, os) {
	var sid = uuid();
	Analytics.sendEvent(guid, mid, eventdata, 'ti.start', deploytype, sid, os);
	return new Session(guid, mid, sid, deploytype, os);
};

/**
 * send an analytics event to the Analytics API
 */
Analytics.sendEvent = function sendEvent (guid, mid, eventdata, event, deploytype, sid, os, immediate, callback) {
	if (mid && typeof(mid) === 'function') {
		callback = mid;
		mid = eventdata = event = deploytype = sid = os = immediate = undefined;
	}
	if (eventdata && typeof(eventdata) === 'function') {
		callback = eventdata;
		eventdata = event = deploytype = sid = os = immediate = undefined;
	}
	if (event && typeof(event) === 'function') {
		callback = event;
		event = deploytype = sid = os = immediate = undefined;
	}
	if (deploytype && typeof(deploytype) === 'function') {
		callback = deploytype;
		deploytype = sid = os = immediate = undefined;
	}
	if (sid && typeof(sid) === 'function') {
		callback = sid;
		sid = os = immediate = undefined;
	}
	if (os && typeof(os) === 'function') {
		callback = os;
		os = immediate = undefined;
	}
	...
	if (!mid) {
		...
		return Auth.getUniqueMachineID(function (err, id) {
			Analytics.sendEvent(guid, id, eventdata, event, deploytype, sid, os, immediate, callback);
		});
	} else {
		sid = sid || uuid();
		os = os || AppC.userAgent;
