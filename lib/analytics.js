 ...
 */
function sendEvent(aguid, eventName, props, data, callback, immediate) {
	if (!aguid) {
		const error = new Error('missing required guid');
		if (callback) {
	...
	if (!props.mid) {
		...
		return Auth.getUniqueMachineID(function (err, machineId) {
			sendEvent(aguid, eventName, Object.assign(props, { mid: machineId }), data, callback, immediate);
		});
	}
	...
	let event = {
		...
		ver: '3',
		aguid,
		event: eventName || 'app.feature',
		sid: props.sid || uuid(),
		mid: props.mid,
		platform: props.platform || userAgent,
