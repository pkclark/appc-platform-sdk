...
const debug = require('debug')('Appc:sdk:request');
...
module.exports = {
	...
	createRequestCustomResponseHandler,
 ...
 */
function createRequest(session, path, method, callback, mapper, json) {
 ...
 * @return {Object} - request object
 */
function createRequestCustomResponseHandler(session, path, method, responseHandler, json) {
	if (typeof method === 'function') {
...
function _createRequest(session, path, method, responseHandler, json) {
	try {
		if (path[0] === '/') {
			path = require('url').resolve(Env.baseurl, path);
		}
